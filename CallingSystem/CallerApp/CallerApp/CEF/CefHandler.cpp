...
{
    OutputDebugStringA("CreateBrowser\n");
    m_hWnd = hParentWnd;
...
std::string CCefHandler::GetLoadingUrl()
{
...
bool CCefHandler::OnKeyEvent(CefRefPtr<CefBrowser> browser, const CefKeyEvent& event, CefEventHandle os_event)
{
	...
	else if (event.type == KEYEVENT_RAWKEYDOWN && event.windows_key_code == 123)
	{
        ...
        CefRefPtr<CefClient> client = new CCefHandler();
        CefBrowserSettings settings;
...
{
    OutputDebugStringA("OnAfterCreated\n");
    m_browser = browser;
}
...
{
    OutputDebugStringA("OnLoadError\n");
